...
def PrintClassBalance(train_counts: dict, test_counts: dict = None):
    """
    Print class balance information.
    
    Args:
        train_counts: Dictionary mapping class names to training sample counts
        test_counts: Optional dictionary mapping class names to test sample counts
    """
    print("\n" + "="*70)
    ...
    print(f"\n{'Training Set Statistics:':<30}")
    print(f"  {'Total Classes:':<26} {len(train_counts)}")
    print(f"  {'Total Samples:':<26} {sum(counts):,}")
    print(f"  {'Most populated:':<26} {max_samples:,} samples")
    print(f"  {'Least populated:':<26} {min_samples:,} samples")
    print(f"  {'Average per class:':<26} {avg_samples:.1f} samples")
    print(f"  {'Imbalance ratio:':<26} {imbalance_ratio:.2f}x")
    
    if imbalance_ratio > 3:
        print(f"\n  ⚠️  HIGH class imbalance! Weighted sampling STRONGLY recommended.")
    elif imbalance_ratio > 2:
        print(f"\n  ⚡ Moderate class imbalance. Weighted sampling recommended.")
    else:
        print(f"\n  ✅ Classes are reasonably balanced.")
    
    if test_counts:
        test_total = sum(test_counts.values())
        print(f"\n{'Test Set Statistics:':<30}")
        print(f"  {'Total Samples:':<26} {test_total:,}")
