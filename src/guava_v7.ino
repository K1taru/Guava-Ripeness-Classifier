...
const int greenLED  = 3;    // D3 - Unripe indicator (PWM)
const int yellowLED = 5;    // D5 - Ripe indicator (PWM)
const int redLED    = 6;    // D6 - Overripe indicator (PWM)
...
void setup() {
  ...
  // Blink all LEDs twice with 1 second duration and interval at startup
  // Using PWM at 50% brightness to protect LEDs without resistors
  for (int i = 0; i < 2; i++) {
    analogWrite(greenLED, 128);   // 50% brightness (128/255)
    analogWrite(yellowLED, 128);  // 50% brightness (128/255)
...
void performScan() {
  ...
  // Green = Unripe, Yellow = Ripe, Red = Overripe
  String statusText = "";
  const char** currentDishArray;
  int currentDishCount;
  
  if (ripeScore >= 2) {
    // OVERRIPE: Red LED ON, others OFF
    analogWrite(greenLED, 0);
    analogWrite(yellowLED, 0);
    ...
    // RIPE: Yellow LED ON, others OFF
    analogWrite(greenLED, 0);
    analogWrite(yellowLED, 150);  // 60% brightness (150/255)
    analogWrite(redLED, 0);
    statusText = "RIPE";
    ...
    // UNRIPE: Green LED ON, others OFF
